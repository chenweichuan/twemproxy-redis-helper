...
abstract class Channel
{
    ...
    protected function createSocket()
    {
        if (!is_resource($this->socket)) {
            if (!$this->socket = @stream_socket_client(
                ...
                throw new \Kafka\Kafka_Exception($errstr, $errno);
            }
    ...
    final protected function read($size, $stream = null)
    {
        ...
        if ($stream === $this->socket && $this->responseSize < $size) {
            ...
            $this->responseSize = null;
            throw new \Kafka\Kafka_Exception\EndOfStream(
    ...
    )
    {
        ...
        if (!$magic = @unpack('C', $this->read(1, $stream))) {
            throw new \Kafka\Kafka_Exception("Invalid Kafka Message");
        }
        ...
        switch ($compression) {
            ...
            case \Kafka\Kafka::COMPRESSION_NONE:
                ...
                if (crc32($payload) != $crc32) {
                    throw new \Kafka\Kafka_Exception("Invalid message CRC32");
                }
            ...
            case \Kafka\Kafka::COMPRESSION_GZIP:
                ...
                if (strcmp(substr($gzHeader, 0, 2), "\x1f\x8b")) {
                    throw new \Kafka\Kafka_Exception('Not GZIP format');
                }
                $gzmethod = ord($gzHeader[2]);
                $gzflags = ord($gzHeader[3]);
                if ($gzflags & 31 != $gzflags) {
                    throw new \Kafka\Kafka_Exception('Invalid GZIP header');
                }
                if ($gzflags & 1) {
                    // FTEXT
                    $ascii = true;
                ...
                if ($gzflags & 2) {
                    ...
                    if ($hcrc != (crc32($gzHeader) & 0xffff)) {
                        throw new \Kafka\Kafka_Exception('Invalid GZIP header crc');
                    }
                    $gzHeader .= $data;
                }
    ...
    {
        if (!is_resource($this->innerStream)) {
            throw new \Kafka\Kafka_Exception("Invalid inner message stream");
        }
